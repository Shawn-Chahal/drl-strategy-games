...
def tournament(game, model_1_epoch, model_2_epoch, n_mcts_play, match_number):
    ...
    model_1 = tf.keras.models.load_model(os.path.join('objects', f'model_{game.name}_{model_1_epoch:04d}.h5'))
    model_2 = tf.keras.models.load_model(os.path.join('objects', f'model_{game.name}_{model_2_epoch:04d}.h5'))
    game.reset()
...
if __name__ == '__main__':
    ...
    if mode is 'train':
        ...
        if initial_epoch == 0:
            ...
            dict_loss = {'Epoch': [], 'Loss (Total)': [], 'Loss (Policy)': [], 'Loss (Value)': [], 'Loss (L2)': [],
                         'Time [s]': []}
            ...
            replay_buffer = pickle.load(open(os.path.join('objects', f'replay_buffer_{game.name}.pkl'), 'rb'))
        ...
        for epoch in range(initial_epoch + 1, final_epoch + 1):
            ...
            pickle.dump(replay_buffer, open(os.path.join('objects', f'replay_buffer_{game.name}.pkl'), 'wb'))
    ...
    if mode is 'tournament':
        ...
        for epoch_1 in tournament_range:
            for epoch_2 in tournament_range:
                tournament_args.append((game, epoch_1, epoch_2, n_mcts_play, len(tournament_args)))

        print(f'Game: {game.name} | Tournament games: {len(tournament_args)} | n_mcts_play: {n_mcts_play}')
        ...
        print(f'Game: {game.name} | Tournament games: {len(tournament_args)} | n_mcts_play: {n_mcts_play}')
        tournament_results = {i: {'Win': 0, 'Draw': 0, 'Lose': 0} for i in tournament_range}
        for p1_epoch, p2_epoch, game_result in tournament_logs:
            ...
            print(f'Player 1: {p1_epoch:3d} | Player 2: {p2_epoch:3d} | Result: {game_result}')

        print('-------------------------')
        print(f'Game: {game.name} | Tournament games: {len(tournament_args)} | n_mcts_play: {n_mcts_play}')
        for epoch in tournament_range:
            print(f"Epoch {epoch:3d} | Win: {tournament_results[epoch]['Win']} | "
